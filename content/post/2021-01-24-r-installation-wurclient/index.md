...
Many users, however, prefer to install software themselves, e.g., because they like to use a newer version of R than the one in Software Center/WURAppStore. When this post was written the release version of R on [r-project.org](https://www.r-project.org/) was 4.0.3, named: "Bunny-Wunnies Freak Out" (released on 2020-10-10). The post has been updated, with the exception of the screenshots, to latest R version 4.6.0, named: "Because it was There" (Released on 2026-04-24).
...
7. Finally create the directory `C:\ProgramData\R\win-library\4.6`, where user installed packages, via the `install.packages()` command in R, will be stored. The last part of the directory path reflects the major release version of R you are installing and should be adapted for other versions, e.g. for R version 4.7.0 the directory created should end with `4.7`.  To create the aforementioned directory copy (CTRL+C) the following line, paste (CTRL+V) it behind the prompt and execute by pressing return (Enter). In case the directory already exists, the message: `A subdirectory or file C:\ProgramData\R\win-library\4.6 already exists.` will appear.
```sh
mkdir C:\ProgramData\R\win-library\4.6
```

8. The Command Prompt application can now be closed by typing `exit` and executing it by pressing return (Enter).

### Download
At the time this post was written, the release of R was version 4.0.3. It has been updated to the latest release version 4.6.0 of R.

The installer for Windows 10/11 can be downloaded directly from this link: [{{< icon name="download" pack="fas" >}} R 4.6.0 for Windows (ca. 87 MB, 64-bit).](https://cloud.r-project.org/bin/windows/base/old/4.6.0/R-4.6.0-win.exe)
...
1. Right-click the downloaded file **R-4.6.0-win.exe** and select 'WUR - Run with administrative rights'. This file will most likely reside in your Downloads folder of your user account.
...
4. Click on the ‘Next’ button to agree to the terms. After this a window will appear, allowing you to select or choose the destination folder, as shown below, where R version 4.5.2 for Windows should be installed. Change the destination location to `C:\MyPrograms\R\R-4.6.0`, as shown in [Figure 11](#figure-install_r_destination) below for version 4.0.3, by typing the destination path directly into the text field displayed (currently showing `C:\Program Files\R\R-4.6.0`) . Click on the ‘Next’ button to continue.
