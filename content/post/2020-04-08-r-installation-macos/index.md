...
## Download 
At the time this post was written, the latest release of R was version 3.6.3. It has been updated to the latest release version 4.6.0 of R (nicknamed: "Because it was There") for macOS Sonoma (macOS 14) or higher.
...
- For macOS Big Sur (11.x) and higher (__Intel 64-bit__ build): [{{< icon name="download" pack="fas" >}} R 4.6.0  (ca. 104 MB, *notarized and signed* __Intel 64-bit__ build)](https://cloud.r-project.org/bin/macosx/big-sur-x86_64/base/R-4.6.0-x86_64.pkg)
- For macOS Sonoma (14.x) and higher (__Apple silicon arm64__ build for M1, M2, M3, or M4 processor MACS only): [{{< icon name="download" pack="fas" >}} R 4.6.0  (ca. 101 MB, *notarized and signed* __ARM 64-bit__ build)](https://cloud.r-project.org/bin/macosx/sonoma-arm64/base/R-4.6.0-arm64.pkg) <!--Due to an error in the release for the Apple silicon arm64 build, use the above download for the Intel 64-bit version!-->

{{% callout note %}}
If installing from the Downloads folder you must allow the Apple Installer access to that directory. Denying the access will result in you not being able to install any software from there and then you must move the file to a location that is not private such as your home folder.
{{% /callout %}}

## Installation
For installing R on macOS follow these steps:

1. Open the downloaded file, either **R-3.6.3.nn.pkg**, **R-4.2.3.pkg**, **R-4.6.0-x86_64.pkg**, or **R-4.6.0-arm64.pkg** depending or your version of macOS or processor (as explained above). This file will most likely reside in Finder > Downloads (shortcut: &#8997;&#8984;L). The file can more easily be found by switching into List view (shortcut: &#8984;2). To switch to Icon view use the shortcut: &#8984;1. The installer package will resemble the image displayed below in [Figure 3](#figure-icon_r_package) (text underneath can, and probably will, differ!).
