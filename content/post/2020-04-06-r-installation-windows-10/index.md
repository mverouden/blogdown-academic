...
The WUR AppStore currently contains **R 4.5.1 Rcmdr**. **Rcmdr** indicates that, apart from running **R version 4.5.1** by using the default Graphical User Interface (a.k.a. **RGUI**), everything is included to run **R Commander**.
...
### Download
At the time this post was written, the latest release of R was version 3.6.3. It has been updated to the latest release version 4.6.0 of R, named "Because it was There". As of version 4.2.0 the R software is only available as 64-bit, meaning that it will not install on a 32-bit version of Windows 10.
...
1. Open the downloaded file **R-4.6.0-win.exe**. This file will most likely reside in the Downloads folder of your user account.
...
4. Click on the ‘Next’ button to agree to the terms. After this a window will appear, allowing you to select or choose the destination folder, as shown in [Figure 2](#figure-r_destination_folder), where R version 4.6.0 for Windows should be installed. If you are content with the default `C:\Program Files\R\R-4.6.0` click on the ‘Next’ button to continue, otherwise use the ‘B<u>r</u>owse...’ button to navigate to an alternative destination or type the destination path directly into the text field displayed (currently showing `C:\Program Files\R\R-4.6.0`).
...
{{% callout note %}}
Congratulations, :satisfied:, you now have R version 4.6.0 installed on your private desktop or laptop computer!
{{% /callout %}}
