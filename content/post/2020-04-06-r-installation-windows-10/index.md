...
### Download
At the time this post was written, the latest release of R was version 3.6.3. It has been updated to the latest release version 4.2.2 of R, named "Innocent and Trusting". As of version 4.2.0 the R software is only available as 64-bit, meaning that it will not install on a 32-bit version of Windows 10.

The installer for Windows 10/11 can be downloaded directly from this link: [{{< icon name="download" pack="fas" >}} R 4.2.2 for Windows (ca. 76 MB, 64-bit)](https://cloud.r-project.org/bin/windows/base/R-4.2.2-win.exe).
...
1. Open the downloaded file **R-4.2.2-win.exe**. This file will most likely reside in your Downloads folder of your user account.
...
4. Click on the ‘Next’ button to agree to the terms. After this a window will appear, allowing you to select or choose the destination folder, as shown in [Figure 2](#figure-r_destination_folder), where R version 4.2.2 for Windows should be installed. If you are content with the default `C:\Program Files\R\R-4.2.2` click on the ‘Next’ button to continu, otherwise use the ‘B<u>r</u>owse...’ button to navigate to an alternative destination or type the destination path directly into the text field displayed (currently showing `C:\Program Files\R\R-4.2.2`).
...
{{% callout note %}}
Congratulations, :satisfied:, you now have R version 4.2.2 installed on your private desktop or laptop computer!
{{% /callout %}}
